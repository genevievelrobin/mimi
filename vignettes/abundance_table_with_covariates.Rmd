...
X <- covmat(aravo$env, aravo$traits,nrow(aravo$spe), ncol(aravo$spe))
# scale and center the covariates
X <- X[,-c(8,9)] # remove ZoogD
X <- scale(X)
```
...
res <- mimi(aravo$spe, model = "covariates", x = X, var.type = vt,
            lambda1 = 5, lambda2 = 0.05)
df.alpha <- data.frame(var = c(colnames(aravo$env)[-c(8,9)],
                               colnames(aravo$traits)),
                       alpha = as.numeric(res$list.alpha[[20]]))
```
