...
#' @import glmnet
wls.cov <- function(y, x, lambda1, lambda2, weights = NULL, thresh = 1e-5,
...
#' \item{theta}{a (nb individuals) x (nb variables) matrix containing the individual effects}
#' @import FactoMineR
irwls.cov <- function(y, x, var.type, lambda1, lambda2, nlevel = NULL, maxit = 100,
                      mu0 = NULL, alpha0 = NULL, theta0 = NULL, thresh = 1e-5,
                      trace.it = F, offset = F, scale = F, max.rank = 5, vt2 = NULL){
  d <- dim(y)
  n <- d[1]
  p <- d[2]
  y <- as.matrix(y)
  ...
  y.imputed <- y0
  y.imputed[is.na(y0)] <- y[is.na(y0)]
  return(list(y.imputed = y.imputed, param = mu + alpha.mat + theta, mu = mu,
...
#' @import FactoMineR
irwls.lr <- function(y, var.type, lambda1, nlevel = NULL, maxit = 100, theta0 = NULL,
...
#' \item{alpha}{a (nb groups) x (nb variables) matrix containing the group effects}
#' \item{theta a}{(nb individuals) x (nb variables) matrix containing the individual effects}
wls.multi <- function(y, groups, lambda1, lambda2, weights = NULL,
  ...
  while(((error > thresh) && (iter < maxit))){
    ...
    alpha.tmp <- alpha
    theta.tmp <- theta
    if(offset) mu <- colMeans(weights * (y - alpha.rep - theta.tmp), na.rm = T) else mu <- 0
    ...
    objective <- c(objective, sum((1 / 2) * weights * (y0 - t(matrix(rep(mu, n), nrow = p)) - alpha.rep - theta)^2, na.rm = T) + lambda1 * sum(d) +
                     lambda2 * sum(abs(alpha)))
    if(iter == 1) {
      error <- 1
    } else{
  ...
  y.imputed <- y0
  y.imputed[is.na(y0)] <- y[is.na(y0)]
  return(list(y.imputed = y.imputed, param = t(matrix(rep(mu, n), nrow = p)) + alpha.rep + theta,
              mu = mu, alpha = alpha, theta = theta))
}
...
#' \item{theta}{a (nb individuals) x (nb variables) matrix containing the individual effects}
#' @import FactoMineR
irwls.multi <- function(y, groups, var.type, lambda1, lambda2, nlevel = NULL,
                        maxit = 100, mu0 = NULL, alpha0 = NULL, theta0 = NULL,
  ...
  y.imputed <- y0
  y.imputed[is.na(y0)] <- y[is.na(y0)]
  return(list(y.imputed = y.imputed, param = mu + alpha.rep + theta,
              mu = mu, alpha = alpha, theta = theta))
}
